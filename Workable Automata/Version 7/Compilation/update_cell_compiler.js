function compileUpdate(rawCode)
{
	let decodeSugar = parseSugar(rawCode);

	if(decodeSugar === null)
	{
		console.log("(COMPILE UPDATE) DECODING DIDN'T WORK");
		return null;
	}

	// console.log("result of syntax sugar parsing: ");
	// console.log(decodeSugar);

	let result = "";

	result += "function(posR,posC)\n";
	result += "{\n";
	result += "\tlet grid = this.grid;\n";
	result += "\tlet rows = this.rows;\n";
	result += "\tlet cols = this.cols;\n";
	result += "\tlet cell = grid[posR][posC];\n\n";
	result += decodeSugar;
	result += "}";
...
function parseSugar(rawCode)
{
	...
	if(numUsages > 1)
	{
		console.log("(PARSE SUGAR) YOU ARE ONLY ALLOWED TO USE THE LET LOCAL STATEMENT ONCE");
		return null;
	}
	else
		return result;
